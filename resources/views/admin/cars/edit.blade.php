...
@section('content')
    <div class="mx-auto max-w-3xl space-y-6 rounded-3xl border border-gray-200 bg-white p-8 shadow-sm">
        <div>
            <h1 class="text-3xl font-semibold text-gray-900">Edit Mobil</h1>
            <p class="mt-2 text-sm text-gray-500">Perbarui informasi mobil.</p>
        </div>
        ...
        <form action="{{ route('admin.cars.update', $car) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input id="name" name="name" type="text" value="{{ old('name', $car->name) }}" required class="mt-2 block w-full rounded-2xl border border-gray-300 bg-white px-4 py-3 text-sm text-gray-700 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-100" />
                @error('name') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="brand" class="block text-sm font-medium text-gray-700">Brand</label>
                <input id="brand" name="brand" type="text" value="{{ old('brand', $car->brand) }}" required class="mt-2 block w-full rounded-2xl border border-gray-300 bg-white px-4 py-3 text-sm text-gray-700 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-100" />
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                <div>
                    ...
                    <input id="price_per_day" name="price_per_day" type="number" min="0" value="{{ old('price_per_day', $car->price_per_day) }}" required class="mt-2 block w-full rounded-2xl border border-gray-300 bg-white px-4 py-3 text-sm text-gray-700 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-100" />
            ...
            <div>
                ...
                @if($car->image)
                    <div class="mt-4">
                        ...
                        <img src="{{ asset('storage/' . $car->image) }}" alt="{{ $car->name }}" class="w-full max-w-xs h-40 object-cover rounded-2xl border border-gray-300" />
                    </div>
                @endif
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <a href="{{ route('admin.cars.index') }}" class="inline-flex items-center justify-center rounded-full border border-gray-300 bg-white px-5 py-3 text-sm font-semibold text-gray-700 transition hover:bg-gray-50">Kembali</a>
                <button type="submit" class="inline-flex items-center justify-center rounded-full bg-blue-600 px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700">Perbarui Mobil</button>
            </div>
        </form>
    </div>
